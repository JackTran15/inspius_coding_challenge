...
export class init1679653973203 implements MigrationInterface {
    name = 'init1679653973203'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`football_match_schedule\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`createdBy\` varchar(255) NOT NULL DEFAULT 'Admin', \`updated\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedBy\` varchar(255) NOT NULL DEFAULT 'Admin', \`day\` int NOT NULL, \`month\` int NOT NULL, \`year\` int NOT NULL, \`dateValue\` datetime NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`football_match\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`createdBy\` varchar(255) NOT NULL DEFAULT 'Admin', \`updated\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedBy\` varchar(255) NOT NULL DEFAULT 'Admin', \`scoreHome\` int NOT NULL DEFAULT '0', \`scoreAway\` int NOT NULL DEFAULT '0', \`startMatch\` datetime NOT NULL, \`day\` int NOT NULL, \`month\` int NOT NULL, \`year\` int NOT NULL, \`status\` enum ('pending', 'live', 'ff') NOT NULL DEFAULT 'pending', \`home_team_id\` int NOT NULL, \`schedule_id\` int NOT NULL, \`away_team_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
